...
def _build_lane_df(res, scn):
    ...
    # If model provided flows, pass them through and ensure brand/center_idx keys exist.
    for rec in (res.get("transfer_flows") or []):
        rr = dict(rec)
        rr.setdefault("brand", "")
        rr.setdefault("center_idx", None)   # transfer dest is WH, should already be set in model; keep if present
        lanes.append(rr)
    for rec in (res.get("inbound_flows") or []):
        rr = dict(rec)
        rr.setdefault("brand", "")
        rr.setdefault("center_idx", None)   # inbound to Tier-1 has no WH; keep None
        lanes.append(rr)

    # If any model flows existed, we're done.
    if (res.get("transfer_flows") or res.get("inbound_flows")) and len(lanes) > 0:
...
def sidebar(scn):
    ...
    with st.sidebar:
        ...
        with st.expander("🏭 Warehouse Brand Restrictions (optional)", False):
            ...
            if chosen_sites:
                for s in chosen_sites:
                    ...
                    sel = st.multiselect(f"{s} → allowed brands", options=brands, default=default_brands, key=f"wh_{s}_{name}")
                    new_map[s] = sel
